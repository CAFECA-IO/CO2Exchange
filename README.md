...
- **平台內部帳務移轉不構成交易拍賣及移轉管理辦法 §26 的「移轉」。**
  所以內部買賣不消耗「每單位限移轉一次」的額度，只有存入與提領各算一次。
  這一點是資產池模型成立的前提；不然鏈下撮合每成交一次就用掉一次移轉，整個設計不通。
...
**逃生門**（營運方不再提交 root 時，使用者仍能憑最後一個 root 領走，
而且營運方關不掉）在 C 期。觸發門檻已定案：**超過 72 小時沒有新的承諾**
（＝3 個 epoch，和帳戶復原的等待期同一個數量級）。

缺它的話，現在的提領只是「營運方正常時可以提領」——而那正好是最不需要保障的時候。
在商業託管的法律性質下，這一項尤其不能省：破產隔離不是法律給的，是機制給的。
`/custody` 上誠實標示為「尚未開放（機制已在鏈上）」。

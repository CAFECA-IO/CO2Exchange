...
export async function GET() {
  try {
    ...
    const withOp = [["kycRegistry", d.kycRegistry], ["retirementCertificate", d.retirementCertificate], ["listing", d.listing], ["carbonPool", d.carbonPool], ["hook", d.hook]] as const;
    ...
    const [pmOwner, listingPaused, poolPaused, trustedRouter, natOwners, natThreshold, opOwners, opThreshold, delay] = await Promise.all([
      publicClient.readContract({ address: d.poolManager, abi: ownedAbi, functionName: "owner" }),
      ...
      publicClient.readContract({ address: d.hook, abi: hookViewAbi, functionName: "trustedRouter" }),
    ...
    return Response.json({
      matrix, poolManagerOwner: pmOwner, poolManagerOwnerIsTimelock: pmOwner.toLowerCase() === d.timelock.toLowerCase(),
      listingPaused, poolPaused, trustedRouter, swapsEnabled: trustedRouter.toLowerCase() === d.router.toLowerCase(),
